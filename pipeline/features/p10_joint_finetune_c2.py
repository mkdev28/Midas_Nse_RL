...
from pipeline.packaging.p9_gym_env import (
    ...
    MidasDataset, coordinate
)
from models.cross_modal_transformer import CrossModalTransformer

# ── Paths ─────────────────────────────────────────────────────────────────────
CKPT = Path("checkpoints")
LOGS = Path("logs/joint_c2")
LOGS.mkdir(parents=True, exist_ok=True)
device = "cuda" if torch.cuda.is_available() else "cpu"
print(f"Joint Fine-Tuning C2 on : {device}")
...
encoder.eval()
print("C2 Encoder loaded and FROZEN for joint fine-tuning.")

# ── 2. Load the 3 Agents ──────────────────────────────────────────────────────
print("Loading C2 Agents...")
a1_model = SAC.load(CKPT / "c2" / "a1" / "a1_c2_sac_300000_steps.zip", device=device)
a2_model = PPO.load(CKPT / "a2_c2_200000_steps.zip", device="cpu")  # PPO MlpPolicy faster on CPU
a3_model = SAC.load(CKPT / "c2" / "a3" / "a3_c2_sac_50000_steps.zip", device=device)
...
print(f"Starting Custom Joint Fine-Tuning Loop for C2 ({TOTAL_STEPS} steps)...")
...
# ── 5. Save ───────────────────────────────────────────────────────────────────
print("\nSaving Jointly Fine-Tuned Models (C2)...")
c2_out = CKPT / "c2" / "joint"
c2_out.mkdir(parents=True, exist_ok=True)
a1_model.save(str(c2_out / "a1_c2_joint_final"))
a2_model.save(str(c2_out / "a2_c2_joint_final"))
a3_model.save(str(c2_out / "a3_c2_joint_final"))
print("Done.")
