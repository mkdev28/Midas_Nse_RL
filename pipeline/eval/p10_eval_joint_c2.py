...
def main():
    print("=" * 60)
    print("Joint Fine-Tuned Pipeline Evaluation (C2)")
    ...
    print("\n[Loading Joint Fine-Tuned Models (C2)]")
    a1_joint = SAC.load(CKPT / "c2" / "joint" / "a1_c2_joint_final", device=device)
    a2_joint = PPO.load(CKPT / "c2" / "joint" / "a2_c2_joint_final", device="cpu")
    a3_joint = SAC.load(CKPT / "c2" / "joint" / "a3_c2_joint_final", device=device)
    ...
    print(f"{'Joint A1+A2+A3 C2 (100k steps)':<40} {np.mean(sharpes):>7.3f} {np.mean(cagrs)*100:>7.1f}% {np.mean(mdds)*100:>8.1f}%")
